...
def table_summary_highlights(df_raw_dt, df_card_dt):
    try:
        ...
        # print(df_summ_merged_raw.columns)
        ...
        # print(df_summ_merged_not_in_raw.columns)
        ...
        # df_summary_highlights.to_excel("final_xummary.xlsx")

        # print(df_summary_highlights)
        ...
        for index, row in df_summary_highlights.iterrows():
            if row['Test Quantity'] == 0:  # pilot
                for column in dash_columns:
                    df_summary_highlights.at[index, column] = '-'
            ...
            def highlight_row(row):
                ...
                if row['DPT'] == '-':
                    if len(month_order)>2:
                        ...
                        pass
                if row["DPT(" + month_order[-1] + ")"] == '-':
                    remarks_list.append('-')
                elif row["DPT(" + month_order[-1] + ")"] != '-' and row['DPT'] != '-':
